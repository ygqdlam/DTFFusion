...
def sam(img1, img2):    #光谱角相似性，img1真实的，img2预测的
    ...
    # numerical stability
    cos_theta = (inner_product / (img1_spectral_norm * img2_spectral_norm + np.finfo(np.float64).eps)).clip(min=0, max=1)   #clip将数组的值限制在最大值与最小值之间，如果大于1那就默认等于1.
    return np.mean(np.arccos(cos_theta))


def psnr(img1, img2, dynamic_range=255):    #img1是真实的，img2是预测的。
...
def _ssim(img1, img2, dynamic_range=255):
    ...
    img1_ = img1.astype(np.float64)
    img2_ = img2.astype(np.float64)
    kernel = cv2.getGaussianKernel(11, 1.5)  # kernel size 11。这是一个二维高斯函数。
...
def ergas(img_fake, img_real, scale=4):
    ...
    if img_fake_.ndim == 2:
        ...
        mses = ((img_fake_ - img_real_)**2).reshape(-1, img_fake_.shape[2]).mean(axis=0)    #这里不是正经的均方根误差，应该是均方根误差的平方
...
def compare_corr(x_true, x_pred):
    """
    用于计算参考图像和融合图像之间光谱特征的相似度，CC的范围是-1到1，如果参考图像和融合图像相同，则CC的值应接近1，否则，接近-1
    ...
    numerator = np.sum(x_true * x_pred, axis=1).reshape(-1, 1)  #有几个通道就有几个数据
    denominator = np.sqrt(np.sum(x_true * x_true, axis=1) * np.sum(x_pred * x_pred, axis=1)).reshape(-1, 1)
    return (numerator / denominator).mean()     #因为有多个通道，所以取平均值。

def RMSE(x_true, x_pred):
    """
    用于测量地面真是参考图像与融合图像之间的像素值的变化，值越小，越好
