...
class SFE(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        ### 改变通道
        self.conv_head = conv3x3(4, n_feats)
        ...
        self.conv_reduce = nn.Conv2d(n_feats, n_feats, kernel_size=(7,7),
    ...
    def forward(self, x):
        ...
        x = x + x1
        ### 缩小尺寸
        x_40 = self.conv_reduce(x)
        x_20 = self.conv_reduce(x_40)
...
class CSFI2(nn.Module):
    ...
    def forward(self, x1, x2):
        x12 = F.interpolate(x1, scale_factor=2, mode='bicubic',align_corners=True)
...
class CSFI3(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        x12 = F.interpolate(x1, scale_factor=2, mode='bicubic',align_corners=True)
        x12 = F.relu(self.conv12(x12))
        x13 = F.interpolate(x1, scale_factor=4, mode='bicubic',align_corners=True)
        x13 = F.relu(self.conv13(x13))

        x21 = F.relu(self.conv21(x2))
        x23 = F.interpolate(x2, scale_factor=2, mode='bicubic',align_corners=True)
...
class MergeTail(nn.Module):
    def __init__(self, n_feats):
        ...
        self.conv_tail2 = conv1x1(n_feats//2, 4)
...
class MainNet(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        ### stage11
        self.conv11_head = conv3x3(256+n_feats+64, n_feats)
        ...
        self.conv22_head = conv3x3(128+n_feats+64, n_feats)
        ...
        ### 改变通道数 64改为64*2
        self.conv33_head = conv3x3(64*2+n_feats+64, n_feats)
        ...
        ###7月18号
        self.conv1_concate = conv3x3(n_feats+256, n_feats)
        self.conv2_concate = conv3x3(n_feats+128, n_feats)
        self.conv3_concate = conv3x3(n_feats+64, n_feats)
    ...
    def forward(self, x, S=None, T_lv3=None, T_lv2=None, T_lv1=None,ref_lv1 = None, ref_lv2 = None, ref_lv3 = None):

        ### shallow feature extraction  提取LR的细节特征。
        x20,x40,x_original = self.SFE(x)
        ...
        if self.should_reset:
            ...
            x1 = x20  ###20*20
            save_list = []
            save_list.append(x1)  ###20*20
            save_list.append(x2)  ###40*40
            save_list.append(x3)  ###80*80
            self.last_hidden = save_list
        ...
        x11_res = torch.cat((x11_res, T_lv3,self.last_hidden[0]), dim=1)
        ...
        ### 7月18号
        x11_res = self.conv1_concate(torch.cat((x11_res, ref_lv3), dim=1))
        ...
        ### soft-attention
        x22_res = x22
        x22_res = torch.cat((x22_res, T_lv2,self.last_hidden[1]), dim=1)
        x22_res = self.conv22_head(x22_res) #F.relu(self.conv22_head(x22_res))
        x22_res = x22_res * F.interpolate(S, scale_factor=2, mode='bicubic',align_corners=True)
        x22 = x22 + x22_res

        x22_res = x22
        ### 7月18号
        x22_res = self.conv2_concate(torch.cat((x22_res, ref_lv2), dim=1))
        ...
        ## soft-attention
        x33_res = x33
        x33_res = torch.cat((x33_res, T_lv1,x_original,self.last_hidden[2]), dim=1)
        x33_res = self.conv33_head(x33_res) #F.relu(self.conv33_head(x33_res))
        x33_res = x33_res * F.interpolate(S, scale_factor=4, mode='bicubic',align_corners=True)
        x33 = x33 + x33_res

        x33_res = x33
        ### 7月18号
        x33_res = self.conv3_concate(torch.cat((x33_res, ref_lv1), dim=1))
        ...
        save_list= []
        save_list.append(x31)   ###20*20
        save_list.append(x32)   ###40*40
        save_list.append(x33)   ###80*80
        self.last_hidden = save_list
